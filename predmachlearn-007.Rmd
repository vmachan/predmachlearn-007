...
A few kinds of techniques were tried to assess what kind of model would work better. The caret train function was used for all of these
...
After predicting the values we then compute the percentage accuracy. We see that the model prediction is accurate __`r sum(predRF==test90pct$classe)/(nrow(test90pct)) * 100`%__ of the time. The plot A in appendix shows the error rate when classifying each of the different classe's.

The second plot (B) in the appendix shows 2 figures showing the impact of variables on the accuracy of the classification done by the model based on Gini index.
