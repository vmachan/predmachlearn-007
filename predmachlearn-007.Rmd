...
The fitted model has an accuracy of 0.961 or 96.1% which was obtained by trying `r modRF$finalModel$mtry` variables (mtry) at each point of classification in the algorithm.
...
After predicting the values we then compute the percentage accuracy. We see that the model prediction is accurate `r sum(predRF==test90pct$classe)/(nrow(test90pct)) * 100`% of the time. The plot A in appendix shows the error rate when classifying each of the different classe's.
